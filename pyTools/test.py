...
def testonPreJoin(player): #testonPlayerJoin
	printLog(f"[TEST] Listener onPreJoin s_player.name = " + player.name)
...
def testonUseItemEx(e):
	printLog(f"[TEST] Listener onUseItemEx i_e['itemid'] = {e['itemid']}, i_e['player'].pos = {listToStr(e['player'].pos)}")
